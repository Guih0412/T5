...
const ExcluirProduto: React.FC<Props> = ({ show, onHide }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3000/produtos/${idNum}`, {
        method: "DELETE",
      });
    ...
    <Modal show={show} onHide={back} centered size="lg">
      ...
      <Modal.Body className="modalBody">
        {step === 1 && (
          <Form>
            <Form.Group className="mb-3">
              <Form.Label>Digite o ID do produto a ser excluído</Form.Label>
              <Form.Control
                type="text"
                value={id}
        ...
        {step === 2 && (
          <>
            <p>
              Tem certeza que deseja excluir o produto com ID <strong>{id}</strong>?
              <br />
